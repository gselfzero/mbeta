...
define("DB_HOST", "localhost"); //Сервер базы данных
define("DB_USERNAME", "gself_2"); //Пользователь базы данных
define("DB_PASSWORD", "vfrcbv22"); //Пароль от пользователя базы данных
define("DB_NAME", "gself_2"); //Имя базы данных
define("APP_SECRET", "Qw2jml9UaT6knL2qdowA"); //Защищенный ключ приложения
define("SERVICE_KEY", "cc8154e4cc8154e4cc8154e4eaccefe751ccc81cc8154e4917c7c7741b5e113d8496e96");
        ...
        if(isset($upgrade)) {
            ...
            if($user->clicks >= $price) {
               if($data['type'] === "0")  $user->speed = (float) $user->speed + (float) $upgrade->speed;
                ...
                $user = R::load('users', R::store($user));            
                $price_new = round((float) $price + ((float) $price / 100) * 50, 3);    
